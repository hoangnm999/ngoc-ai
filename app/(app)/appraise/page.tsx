...
const NATURAL_CFG: Record<string, { color: string; bg: string; icon: string }> = {
...
const GUARD_CFG: Record<HallucinationLevel, { color: string; bg: string; border: string; icon: string; label: string }> = {
  SAFE:    { color: '#065F46', bg: '#ECFDF5', border: '#6EE7B7', icon: '✓', label: 'Nhận diện đáng tin cậy' },
  WARNING: { color: '#92400E', bg: '#FEF3C7', border: '#FCD34D', icon: '⚠', label: 'Nhận diện sơ bộ — cần xác nhận thêm' },
  BLOCKED: { color: '#991B1B', bg: '#FEE2E2', border: '#FCA5A5', icon: '✕', label: 'Ảnh chưa đủ để nhận diện' },
}
...
function GuardBanner({ guard }: { guard: HallucinationGuard }) {
  ...
  return (
    <div style={{ padding: '14px 18px', borderRadius: 10, background: cfg.bg, border: `1px solid ${cfg.border}`, marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: guard.reasons.length ? 10 : 0 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>AI Confidence Guard · {guard.level}</div>
        </div>
      </div>
      {guard.reasons.map((r, i) => (
        <div key={i} style={{ fontSize: 15, color: cfg.color, padding: '3px 0 3px 28px', lineHeight: 1.5, opacity: 0.85 }}>› {r}</div>
      ))}
      {guard.suggestion && (
        <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 6, background: 'rgba(0,0,0,.04)', fontSize: 15, color: 'var(--text-2)', borderLeft: `3px solid ${cfg.border}` }}>
          💡 {guard.suggestion}
        </div>
      )}
    </div>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
          {[
            ...
            <div key={tip.title} style={{ padding: '12px 14px', borderRadius: 8, background: 'var(--bg-3)', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: 18, marginBottom: 4 }}>{tip.icon}</div>
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', marginBottom: 2 }}>{tip.title}</div>
              <div style={{ fontSize: 12, color: 'var(--text-2)' }}>{tip.desc}</div>
            </div>
          ))}
        </div>
      </Card>
    ...
    <div style={{
      ...
      {result && (
        <div>
          ...
          {/* Tự nhiên / giả */}
          {natureCfg && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, padding: '8px 12px', borderRadius: 8, background: natureCfg.bg, border: `1px solid ${natureCfg.color}30` }}>
              <span style={{ fontSize: 14, color: natureCfg.color }}>{natureCfg.icon}</span>
              <span style={{ fontSize: 15, color: natureCfg.color, fontWeight: 600 }}>{result.muc_do_tu_nhien}</span>
              <span style={{ marginLeft: 'auto', fontSize: 12, color: natureCfg.color, fontFamily: 'var(--font-mono)', fontWeight: 500 }}>{result.do_tin_cay}%</span>
            </div>
          )}
          ...
          <div style={{ fontSize: 13 }}>
            ...
            {result.canh_bao_co_the_gia && (
              <InfoRow label="⚠ Cảnh báo" value={result.canh_bao_co_the_gia} color="#9A3412" />
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 24px 80px' }}>
            ...
            return (
              <div key={slot.id}
                ...
                style={{
                  ...
                  border: `1.5px ${img ? 'solid' : 'dashed'} ${img && !img.ok ? '#FCD34D' : img ? 'rgba(13,148,136,.3)' : '#D1D5DB'}`,
                  background: img ? 'var(--bg-3)' : '#FAFAFA',
                }}
              >
                ...
                {img ? (
                  <>
                    ...
                    {!img.ok && <div style={{ position: 'absolute', top: 5, left: 5, background: '#D97706', borderRadius: 4, padding: '1px 6px', fontSize: 11, color: '#fff', fontWeight: 700 }}>NHỎ</div>}
                    <button onClick={e => { e.stopPropagation(); setImages(p => { const n = { ...p }; delete n[slot.id]; return n }) }}
                      style={{ position: 'absolute', top: 5, right: 5, background: 'rgba(255,255,255,.9)', border: 'none', color: '#DC2626', borderRadius: 5, width: 22, height: 22, fontSize: 15, fontWeight: 700 }}>×</button>
                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '10px 6px 4px', background: 'linear-gradient(transparent,rgba(0,0,0,.5))', fontSize: 11, color: '#fff', fontFamily: 'var(--font-mono)' }}>{img.w}×{img.h}</div>
                  </>
                ) : (
                  <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 10 }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--text-3)', textAlign: 'center', fontWeight: 500 }}>{slot.label}</span>
                    <span style={{ fontSize: 11, color: 'var(--text-3)', textAlign: 'center' }}>{slot.hint}</span>
                  </div>
                )}
              </div>
            )
      ...
      <Card style={{ padding: 20, marginBottom: 24 }}>
        <Label>Video quay ngọc <span style={{ color: 'var(--text-3)', fontSize: 11, textTransform: 'none' }}>— tùy chọn</span></Label>
          ...
          {vidLoading
            ...
            : frames.length > 0
              ? <>
                  ...
                  <div>
                    <div style={{ fontSize: 15, color: 'var(--jade)', fontWeight: 600 }}>✓ {frames.length} frames</div>
                    <button onClick={e => { e.stopPropagation(); setFrames([]) }} style={{ background: 'none', border: 'none', color: 'var(--text-3)', fontSize: 12, padding: 0 }}>× Xoá</button>
                  </div>
                </>
      ...
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        ...
        <Btn variant="jade" onClick={analyze} disabled={!canAnalyze || loading} style={{ padding: '14px 52px', fontSize: 16 }}>
          {loading
            ? <><Spinner size={16} color="#fff" />AI đang nhận diện…</>
      ...
      {result && (
        <div className="fade-up">
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <span style={{ fontSize: 16, color: cfg.color, fontWeight: 600 }}>{cons.muc_do_tu_nhien}</span>
                        </div>
                      )
                    })()}
                  </div>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            <AICard name="Claude Sonnet" color="#8B5CF6" vendor="Anthropic" role="Phân tích chuyên sâu"
              result={result.sonnet} error={result.errors?.sonnet} />
            <AICard name="Claude Haiku" color="#3B82F6" vendor="Anthropic" role="Xác thực nhanh"
          ...
          {result.sonnet && (
            <Card style={{ padding: 24, marginBottom: 20 }}>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <InfoRow label="Nên kiểm định" value={result.sonnet.nen_kiem_dinh} color="var(--jade)" />
                <InfoRow label="Lưu ý khi mua" value={result.sonnet.luu_y_khi_mua} color="#92400E" />
          ...
          {result.partial_errors && Object.keys(result.partial_errors).length > 0 && (
            <div style={{ marginBottom: 16, padding: '10px 14px', borderRadius: 8, background: '#FEF3C7', border: '1px solid #FCD34D' }}>
              <div style={{ fontSize: 12, color: '#92400E', marginBottom: 4, fontWeight: 600 }}>⚠ Một số AI không phản hồi:</div>
              {Object.entries(result.partial_errors).map(([ai, err]) => (
                <div key={ai} style={{ fontSize: 12, color: '#78350F' }}>{ai}: {err}</div>
              ))}
            </div>
          )}
