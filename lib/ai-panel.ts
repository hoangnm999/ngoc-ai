...
export interface ConsensusResult {
  thap: number; cao: number; xep_hang: string
  do_tin_cay: number; agreement: number
}

export interface PanelResult {
  sonnet: AIResult | null; haiku: AIResult | null; gemini: AIResult | null
...
Phân tích TỔNG HỢP tất cả ảnh. Nếu có thông tin khai báo, hãy đối chiếu với ảnh và chỉ ra mâu thuẫn nếu có.
...
function buildHaikuSystem(declContext?: string) {
  const decl = declContext ? `\n\n${declContext}\n` : ''
  return `Bạn là chuyên gia xác thực ngọc đá quý, chuyên phát hiện hàng giả và xử lý hóa học.${decl}
Nếu có thông tin khai báo, hãy kiểm tra xem ảnh có phù hợp không.
...
function buildGeminiPrompt(declContext?: string) {
  const decl = declContext ? `\n\nSELLER DECLARATION:\n${declContext}\n\nPlease cross-check the declaration against the images and note any discrepancies in mau_thuan_khai_bao field.\n` : ''
  return `You are an expert gemologist. Analyze these gemstone images objectively.${decl}
